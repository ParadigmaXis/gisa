...
{	
	...
	/// <summary>
	/// Summary description for WOutlookBar.
	/// </summary>
	public class WOutlookBar : System.Windows.Forms.UserControl
	{
		...
		private int       m_ActiveBarIndex     = 0;
		//private int       m_DefaultBarHeight   = 20;
     ...
     //		OnViewStyleChanged(e);
		...
		protected override void OnPaint(System.Windows.Forms.PaintEventArgs e)
		{
			...
			e.Graphics.Clear(m_ViewStyle.BarClientAreaColor1);
			e.Graphics.FillRectangle(new System.Drawing.Drawing2D.LinearGradientBrush(this.ClientRectangle, m_ViewStyle.BarClientAreaColor1, m_ViewStyle.BarClientAreaColor2, System.Drawing.Drawing2D.LinearGradientMode.Vertical),this.ClientRectangle);
		...
		{			
			foreach(Bar bar in m_pBars){
                ...
                if(bar.BarClientRect.Height > 0){
					DrawVisibleBarItems(g);
				}
			}
		}
	...
	//		int requiredHeight = 0;
			Bar bar = m_pBars[m_ActiveBarIndex];
			...
			if(bar.Items.Count > 0){            
				...
				for(int i=bar.FirstVisibleIndex;i<bar.Items.Count;i++){
					...
					Point mPt = this.PointToClient(Control.MousePosition);
					bool hot  = item.Bounds.Contains(mPt) && bar.ItemFullyVisible(item) && !this.UpScrollBtnRect.Contains(mPt)&& !this.DownScrollBtnRect.Contains(mPt);				    
	...
	//				requiredHeight += item.Bounds.Height;				
		...
		{
			Bar bar   = this.Bars[m_ActiveBarIndex];
			Point mPt = this.PointToClient(Control.MousePosition);
		...
		public Item StuckenItem
		{
			...
			set{ this.Invalidate(m_StuckenItem.Bounds); m_StuckenItem = value; this.Invalidate(m_StuckenItem.Bounds); }
		}
		...
		{
            get { return this.ActiveBar != null && !this.ActiveBar.IsLastVisible; }				
		}
